...
The Announce Date defines a release for each component. If you save a component update with the same Announce Date later, the app updates that existing release record instead of creating a duplicate. Blank fields retain the previously saved values for that component and Announce Date, so adding a later milestone does not wipe out version, Dev, PPMO, PROD, notes, or status values. Status is release-level: when one component's status changes, every component with the same Announce Date is updated to that status. You can also use the Release tab's Load Existing Release field, or pick the same component and Announce Date, to pull a saved row back into the form and edit it directly.
